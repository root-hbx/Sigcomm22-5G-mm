...
DATA_FOLDER = path.join(data_processed_dir, 'figure14-16k_and_volumetric_video')
VOD_DATA_FOLDER = path.join(DATA_FOLDER, '16k-vod-emulation')
VOL_DATA_FOLDER = path.join(DATA_FOLDER, 'volumetric-emulation')

## preprocess vod data
data_f = open(path.join(VOD_DATA_FOLDER, 'summary.json'))
data_vod = json.load(data_f)
data_pred = open(path.join(VOD_DATA_FOLDER, 'predict_summary.json'))
data_vod_pred = json.load(data_pred)
colormap = {'RBHO': 'navy', 'robustMPCGT': 'violet', 'RB': 'brown', 'robustMPC': 'darkgreen', 'robustMPCHO': 'r',
            'fastMPC': 'darkorange', 'fastMPCHO': 'b',
...
f = open(path.join(VOL_DATA_FOLDER, 'quality_improve.json'))
quality_improve = json.load(f)
f.close()
f = open(path.join(VOL_DATA_FOLDER, 'stall_improve.json'))
...
if True:  # use truth value to turn on and off plotting
    plot_id = '14'
    plot_name = 'volumetric-vod-qoe'
    plt.close('all')
    fig = plt.figure(figsize=(18, 4.5), constrained_layout=False)
    subfigs = fig.subfigures(1, 3, width_ratios=[3.5, 3, 3.5])
    ax0, ax1, ax2 = subfigs.flat[0].subplots(3, 1, sharex=True), subfigs.flat[1].subplots(1, 1), subfigs.flat[
        2].subplots(1, 2, sharey=True)
    ...
    for key in quality_improve.keys():
        ...
        eb = myax.errorbar(stall_improve[key][0], quality_improve[key][0], xerr=stall_improve[key][1],
                           yerr=quality_improve[key][1], color=colors[key], capsize=6, lw=2, capthick=2)
    ...
    ax0[2].annotate("Better QoE", fontsize=fontsize, horizontalalignment="center", xy=(1.02, 0.48), xycoords='data',
                    xytext=(1.27, 0.25), textcoords='data',
                    arrowprops=dict(arrowstyle="->, head_width=0.3", connectionstyle="arc3", lw=3))

    map_scheme_ax = {'RBHO': 2, 'robustMPCGT': 1, 'RB': 2, 'robustMPC': 1, 'robustMPCHO': 1, 'fastMPC': 0,
                     'fastMPCHO': 0,
                     'fastMPCGT': 0, 'RBHOGT': 2}
    for scheme in schemes:
        selected_ax = map_scheme_ax[scheme]
        line = ax0[selected_ax].errorbar(data_vod[scheme][0], data_vod[scheme][1], xerr=data_vod[scheme][2],
                                         yerr=data_vod[scheme][3], capsize=3, color=colormap[scheme])
        if 'GT' in scheme:
        ...
        if scheme == 'robustMPC':
            ax0[1].annotate("robustMPC", (data_vod[scheme][0] + 0.5, data_vod[scheme][1] - 0.11),
                            fontsize=fontsize_label, color=colormap[scheme])
        elif scheme == 'fastMPC':
            ax0[0].annotate("fastMPC", (data_vod[scheme][0] + 0.35, data_vod[scheme][1] + 0.04),
                            fontsize=fontsize_label, color=colormap[scheme])
        elif scheme == 'FESTIVE':
            ax0[0].annotate("FESTIVE", (data_vod[scheme][0] + 0.35, data_vod[scheme][1] - 0.15),
                            fontsize=fontsize_label, color=colormap[scheme])
        elif scheme == 'robustMPCGT':
            ax0[1].annotate("robustMPC-GT", (data_vod[scheme][0] + 0.2, data_vod[scheme][1] + 0.12),
                            fontsize=fontsize_label, color=colormap[scheme])
            ...
            ax0[0].annotate("BOLA", (data_vod[scheme][0] - 0.05, data_vod[scheme][1] - 0.12), fontsize=fontsize_label,
                            color=colormap[scheme])
        elif scheme == 'RBHOGT':
            ax0[2].annotate("RB-GT", (data_vod[scheme][0] - 0.05, data_vod[scheme][1] - 0.12), fontsize=fontsize_label,
                            color=colormap[scheme])
        else:
            ax0[0].annotate(scheme, (data_vod[scheme][0] - 0.02, data_vod[scheme][1] + 0.01), fontsize=fontsize_label,
                            color=colormap[scheme])
    ...
    hd1 = ax1.bar([0], [data_vod_pred['w/o HO\n(fastMPC)'][0]], 0.35, color='none', edgecolor='darkred',
                  ecolor='darkred', hatch='/', linewidth=2,
                  yerr=data_vod_pred['w/o HO\n(fastMPC)'][1], error_kw=dict(lw=2.5, capsize=5, capthick=2),
                  label='fastMPC')
    hd2 = ax1.bar([0.5], [data_vod_pred['w/ HO\n(fastMPC)'][0]], 0.35, color='none', edgecolor='darkred',
                  ...
                  yerr=data_vod_pred['w/ HO\n(fastMPC)'][1], error_kw=dict(lw=2.5, capsize=5, capthick=2),
                  label='fastMPC')
    hd3 = ax1.bar([1], [data_vod_pred['w/o HO\n(fastMPC-PR)'][0]], 0.35, color='none', edgecolor='darkgreen',
                  ecolor='darkgreen', hatch='x', linewidth=2,
                  yerr=data_vod_pred['w/o HO\n(fastMPC-PR)'][1], error_kw=dict(lw=2.5, capsize=5, capthick=2),
                  label='fastMPC-PR')
    ...
    ax1.legend(handles=[hd1, hd3], loc='upper right', fontsize=24, borderpad=0.0, handlelength=1.2, edgecolor='white',
